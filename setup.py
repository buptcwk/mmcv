...
def get_extensions():
    ...
    if os.getenv('MMCV_WITH_TRT', '0') != '0':
        ...
        extra_compile_args['nvcc'] = [cuda_args, '-std=c++17', '-arch=sm_90'] if cuda_args else []
    ...
    if EXT_TYPE == 'parrots':
        ...
        cuda_args = os.getenv('MMCV_CUDA_ARGS')
        extra_compile_args = {
            'nvcc': [cuda_args, '-std=c++17', '-arch=sm_90'] if cuda_args else ['-std=c++14'],
            'cxx': ['-std=c++14'],
        }
        ...
        if 'nvcc' in extra_compile_args and platform.system() != 'Windows':
            extra_compile_args['nvcc'] += ['-std=c++17', '-arch=sm_90']
